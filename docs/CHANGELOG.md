...
## 2026-09-26 — 障礙券、券的播報與賽後統計、下注金額調整、埠衝突與開場等待修正

### 需求與問題

1. 大螢幕下方一直顯示「中繼伺服器意外結束，重啟中」，但 QRCode 有出來，最後才出現「大螢幕已連線」
2. 開場動畫播完時 QRCode 還沒好
3. 手機重新整理後不用重新進場，詢問是什麼機制（見下方說明，程式未改）
4. 下注金額改為 1／10／100／1000／全部／自訂，取消最低下注
5. 手機結算畫面沒有每匹馬的完賽時間
6. 新增障礙券：出現在馬的前方，撞到後 2 秒不能動，10 籌碼
7. 有人用券時，畫面上要顯示誰用了什麼券
8. 結算時每匹馬要列出不同玩家分別用了什麼券、用了幾張

### 根本原因

**1. 埠被佔用卻偵測不到。** Editor.log 顯示該次 Play 中 node 連續 24 次（約 70 秒）以 `EADDRINUSE` 結束。
啟動器原本用「連得上 127.0.0.1:8080 嗎」判斷埠是否有人用，但佔住埠的程式並沒有在接受連線
（例如綁了埠卻沒在聽，或正在關閉中的舊程式），連線探測以為空著、node 卻綁不上，只能一直重試。
QRCode 仍出現是因為外網通道獨立運作。以「綁住所有位址但不 listen」的 socket 完整重現了此現象。
另外 Windows 的 netstat 看不到這類 socket，所以當時也無從得知佔用者。

**2. 開場長度短於外網通道就緒所需時間。** 預設開場 6 秒，外網通道需要約 8～16 秒（等 DNS 生效）。

**3. 重新整理會記住身分是刻意的設計**（CLAUDE.md 規定）：手機第一次開頁面時產生玩家識別碼並與暱稱一起存在
瀏覽器的 localStorage，重新整理或斷線回來時自動用同一個識別碼重新進場，大螢幕認得同一個帳戶與籌碼。

### 修改的檔案與內容

**Core**
- `ItemShop.cs`：新增 `ItemKind`（加速／減速／障礙），券的 API 改用它；障礙券規則（前方已有障礙、
  被絆住或保護期內、太靠近終點皆拒絕）；每次成功使用記錄到 `Uses`，`Summarize()` 彙整成
  「閘號 × 券種 × 玩家 → 張數」
- `RaceEngine.cs`：`PlaceObstacle()`；撞到障礙物時停在障礙物位置、速度歸零、停住指定秒數後從靜止重新加速；
  停住期間照常推進速度波動（共用亂數，避免影響其他馬）；衝線時撤掉沒撞到的障礙物
- `HorseState.cs`：`ObstacleAt`、`ObstacleStunSeconds`、`StunRemaining`、`StunEndedAt`、`ObstacleHits`（含 Clone）
- `Config/ItemConfig.cs`：`ObstacleCost = 10`、`ObstacleStunSeconds = 2`、`ObstacleLeadMeters = 10`、
  `ObstacleImmunitySeconds = 3`、`ObstacleCooldownSeconds = 0`（＝停住秒數）；`CostOf()`、`CooldownOf()`
- `Config/RaceConfig.cs`：`MinimumBet` 50 → 1
- `Config/PresentationConfig.cs`：`IntroMaxWaitSeconds = 30`
- `Protocol/Messages.cs`：`ItemKinds` 加 `obstacle`；`phase` 加障礙券規則；`wallet` 加 `obstacleCool`；
  `drive` 旗標加 4（被絆住）、8（前方有障礙物）；`result` 加 `times` 與 `usage`

**Net**
- `PortPicker.cs`（新增）：啟動前用和 node 相同的方式（所有位址、雙堆疊）實際綁一次埠；綁不上且不是可沿用的伺服器
  就換下一個空的埠，並嘗試用 netstat 查出佔用者寫進 log
- `LocalRelayLauncher.cs`：新增 `Port`（實際使用的埠）；node 回報 EADDRINUSE 時顯示「埠被佔用」而非籠統的意外結束
- `LocalAddress.cs`：`WithPort()`

**View**
- `RaceDirector.cs`：大螢幕連線位址與區網網址改用實際的埠；處理障礙券與新的拒絕原因；
  用券成功時推播到大螢幕；結算廣播完賽時間與券的使用統計；被絆住的馬顯示粉紅光環；
  開場在「大螢幕已連上且外網通道有結果」之後才結束；測試鍵 3 放障礙物
- `ItemFeed.cs`（新增）：右上角即時播報「阿明 → 蒼影 減速券 ×3」，同人同馬同券合併計數，5 秒後淡出
- `ObstacleMarkers.cs`（新增）：賽道上的粉紅白柵欄，位置對準馬鼻前方
- `ItemStyle.cs`（新增）：券的名稱、顏色（與手機一致）與賽後統計文字格式
- `RaceHud.cs`：名次面板每匹馬下方加一行券的使用統計；掛上播報
- `IntroPlayer.cs`：開場內容播完若連線還沒好，預設畫面繼續動／影片停在最後一格，顯示「連線準備中…」，
  最多再等 30 秒

**手機頁**
- 下注金額 1／10／100／1,000／全部＋自訂
- 第三張券「障礙券」（10 籌碼、前方擋路停 2 秒），各自選馬與冷卻動畫
- 場上名次顯示「暈」（被絆住）與「▮」（前方有障礙物）
- 結算畫面顯示每匹馬的完賽秒數，並在下方列出誰用了什麼券幾張

**設定與文件**
- `race.json`：`MinimumBet`、障礙券參數、`IntroMaxWaitSeconds`
- `docs/ARCHITECTURE.md`：道具表、障礙券規則、訊息協定

### 驗證

- Core 單元測試 **206 項全綠**（新增 30 項：障礙券價格與冷卻、放置限制、撞到停在障礙物位置、停住期間不動、
  恢復起跑、**不影響其他馬的跑法**、被絆住的馬明顯落後、購買規則與各種拒絕、保護期、太靠近終點、
  使用紀錄與彙整排序、結算時保留、下一場清空）
- 用 Unity 自帶 Roslyn 編譯 Core／Net／View／Editor：零錯誤、零 C# 警告
- 啟動器埠衝突測試 **9 項全綠**：以「綁住所有位址但不 listen」重現 Unity 裡的現象
  （連線探測以為空著、node 回報 EADDRINUSE），新機制自動改用下一個埠、伺服器一次啟動成功、不再反覆重啟；
  已有伺服器在聽時仍沿用原埠
- 完整外網端對端 42 項全綠（無退步）
- 手機頁 UI 測試 **33 項全綠**（新增：金額選項、障礙券預設目標、送出 obstacle、扣 10 籌碼並冷卻、
  場上「暈」與「▮」、結算的完賽時間與券的使用統計）
- 未能驗證：在 Unity 裡看實際的障礙物、播報與名次面板畫面（需要使用者按 Play）
